VLApp.directive('circularsliderDirective', function () {
    ...
    return {
        ...
        controller: "circularsliderCtrl",

        templateUrl: "templates/circularsliderTemplate.html",
        
        link: function (scope, element, attr, controller, transcludeFn) {
            var sliderdata = scope.fetchComponentData(attr.id);
   ...
   var shapes = {
        "Circle": {
          ...
          deg2Val: function(deg) {
            ...
            var range = parseInt(scope.max) - parseInt(scope.min) + 1;
            if (deg < 0 || deg > 359)
              throw "Invalid angle " + deg;

            deg = (deg + 90) % 360;
            return Math.round(deg * (range / 360.0)) + parseInt(scope.min) ;
          },
          val2Deg: function(value) {
            var scope = cs.components.scope;
            var range = scope.max - scope.min + 1;
            if (parseInt(value) < parseInt(scope.min) || parseInt(value) > parseInt(scope.max))
      ...
      scope.$watch('value', function(v) {
        if (!element[0].offsetParent || cs.components.scope.disabled)
          return;
        if (v !== scope.$$value) {
          try {
            cs.funs.setValue(v);
      ...
      function setValue(value) {
        ...
        var val = scope.clockwise ? parseInt(value) : (parseInt(scope.max) - parseInt(value));
        var d360 = shapes[scope.shape].val2Deg(parseInt(val));
